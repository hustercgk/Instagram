...
public class ImageGLSurfaceView extends GLSurfaceView implements Renderer{
    ...
    public void setFilterWithConfig(final String config) {
        ...
        queueEvent(new Runnable() {
            ...
            public void run() {
                ...
                requestRender();//如果不requestRender的话，就不会把bitmap绘制到GL上。
            }
        });
    }
    public void setFilterWithConfig2(final String config) {
    ...
    public void setFilterIntensity2(final float intensity) {
        if(mImageHandler == null)
            return;
...
//        setZOrderMediaOverlay(true);
    ...
    protected void calcViewport() {
        ...
        System.out.println("eslab " +String.format("View port: %d, %d, %d, %d", mRenderViewport.x, mRenderViewport.y, mRenderViewport.width, mRenderViewport.height));
    }
}
