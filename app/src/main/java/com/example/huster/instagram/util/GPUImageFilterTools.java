...
public class GPUImageFilterTools {
    ...
    public static GPUImageFilter createFilterForType(final Context context, final FilterType type) {
        switch (type) {
            case CONTRAST:
                return new GPUImageContrastFilter(1.4f);
            case GAMMA:
                return new GPUImageGammaFilter(1.4f);
            case INVERT:
            ...
            case SEPIA:
                return new GPUImageSepiaFilter(1.4f);
            case SHARPEN:
                GPUImageSharpenFilter sharpness = new GPUImageSharpenFilter();
                sharpness.setSharpness(1.6f);
                return sharpness;
            ...
            case THREE_X_THREE_CONVOLUTION:
                ...
                return convolution;
            case EMBOSS:
                return new GPUImageEmbossFilter(2.7f);
            case POSTERIZE:
            ...
            case SATURATION:
                return new GPUImageSaturationFilter(1.4f);
            case EXPOSURE:
                return new GPUImageExposureFilter(0.0f);
            case HIGHLIGHT_SHADOW:
            	return new GPUImageHighlightShadowFilter(0.7f, 0.7f);
            case MONOCHROME:
            	return new GPUImageMonochromeFilter(0.7f, new float[]{0.6f, 0.45f, 0.3f, 1.0f});
            case OPACITY:
                return new GPUImageOpacityFilter(1.0f);
            case RGB:
                return new GPUImageRGBFilter(1.0f, 1.0f, 1.0f);
            case WHITE_BALANCE:
                return new GPUImageWhiteBalanceFilter(4200.0f, 0.0f);
            case VIGNETTE:
            ...
            case HAZE:
                return new GPUImageHazeFilter(0.12f, 0.12f);
            case LAPLACIAN:
    ...
    private static GPUImageFilter createBlendFilter(Context context, Class<? extends GPUImageTwoInputFilter> filterClass) {
        try {
            ...
            filter.setBitmap(BitmapFactory.decodeResource(context.getResources(), R.drawable.head));
            return filter;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
    ...
    public static class FilterList {
        ...
        public List<Boolean> textColorFlags = new LinkedList<>();
        public Context mContext;
        private FilterType filterType;
        public void addFilter(final String name, final FilterType filter) {
            names.add(name);
            textColorFlags.add(false);
            filters.add(filter);
        }
        ...
        public void setImage(Bitmap bitmap){
            ...
            textColorFlags.set(0, true);
            bitmaps.add(image.getBitmapWithFilterApplied());
            for(int i = 1; i<names.size(); i++){
                image.setFilter(createFilterForType(mContext, filters.get(i)));
                bitmaps.add(image.getBitmapWithFilterApplied());
            }
        }
    }
    public static FilterList getFilterList(Context context){
        FilterList filters = null;
        if(filters==null){
            filters = new FilterList(context);
            filters.addFilter("Normal", FilterType.CONTRAST);
            filters.addFilter("Contrast", FilterType.CONTRAST);
            filters.addFilter("Gamma", FilterType.GAMMA);
            filters.addFilter("Brightness", FilterType.BRIGHTNESS);
            filters.addFilter("Sepia", FilterType.SEPIA);
            ...
            filters.addFilter("Vignette", FilterType.VIGNETTE);
            filters.addFilter("ToneCurve", FilterType.TONE_CURVE);
            filters.addFilter("Lookup", FilterType.LOOKUP_AMATORKA);
            filters.addFilter("Dilation", FilterType.DILATION);
            filters.addFilter("Smooth Toon", FilterType.SMOOTH_TOON);
            filters.addFilter("Haze", FilterType.HAZE);
        }
